...
func TestAddRemoveMember(t *testing.T) {
	...
	readConfig, err := leader.GetMembers(context.TODO())
	require.NoError(t, err)
	...
	newConfig, err := leader.AddMember(context.TODO(), memberID1, memberAddr1)
	require.NoError(t, err)
	require.True(t, newConfig.IsMemberByID(memberID1))
	memberID2 := "member-2"
	memberAddr2 := "127.0.0.3:8080"
	newConfig, err = leader.AddMember(context.TODO(), memberID2, memberAddr2)
	require.NoError(t, err)
	require.True(t, newConfig.IsMemberByID(memberID2))
	memberID3 := "member-3"
	memberAddr3 := "127.0.0.4:8080"
	newConfig, err = leader.AddMember(context.TODO(), memberID3, memberAddr3)
	require.NoError(t, err)
	...
	newConfig, removed, err := leader.RemoveMember(context.TODO(), memberID1)
	require.NoError(t, err)
	...
	newConfig, removed, err = leader.RemoveMember(context.TODO(), memberID3)
	require.NoError(t, err)
	require.NotNil(t, removed)
